...
namespace 悲愴三国志Zero2_1.Code {
	internal static class UpdateGame {
		...
		internal static Game PayAttackFunds(Game game,ECountry country) => game with { CountryMap=game.CountryMap.MyUpdate(country,(_,countryInfo) => countryInfo with { Funds=countryInfo.Funds-Country.CalcAttackFunds(game,country) }) };
		...
		internal static Game AreaAttack(Game game,ECountry attackSide,EArea target,Army attack,Army defense,AttackJudge judge) {
			...
			static Game ChangeHasCountry(Game game,ECountry attackCountry,ECountry? defenseCountry,EArea targetArea) {
				return AppendLogMessage(game,[$"{Text.GetCountryText(defenseCountry,Lang.ja)}領の{targetArea}が{attackCountry}領に"]).MyApplyF(game => UpdateAreaMap(game,attackCountry,targetArea)).MyApplyF(game => MakeEmptyPost(game,targetArea));
		...
		private static Game DeathCommander(Game game,CommanderType commander,ERole role) {
			...
			static Game UpdatePersonMap(Game game,List<PersonType> deathPersons) => game with { PersonMap=deathPersons.Aggregate(game.PersonMap,(fold,value) => fold.MyUpdate(value,(_,param) => param with { GameDeathTurn=game.PlayTurn })) };
		}
		internal static Game NextTurn(Game game) {
			...
			static Game InOutFunds(Game game) => game with { CountryMap=game.CountryMap.ToDictionary(v => v.Key,v => v.Value with { Funds=v.Value.Funds+Country.GetInFunds(game,v.Key)-Country.GetOutFunds(game,v.Key) }) };
