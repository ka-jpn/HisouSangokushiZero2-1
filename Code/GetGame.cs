...
namespace 悲愴三国志Zero2_1.Code {
	internal static class GetGame {
		...
		private static Game InitGame(Scenario scenario,ScenarioData.ScenarioInfo? scenarioInfo) => new(scenario,scenarioInfo?.AreaMap.ToDictionary()?? [],scenarioInfo?.CountryMap.ToDictionary()?? [],scenarioInfo?.PersonMap.ToDictionary()?? [],null,null,0,Phase.Starting,null,false,[],[],[],[]);
