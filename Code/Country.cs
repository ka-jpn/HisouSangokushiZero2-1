...
namespace 悲愴三国志Zero2_1.Code {
	internal static class Country {
		...
		internal static decimal GetInFunds(Game game,ECountry country) => GetAreaNum(game,country)==0 ? 0 : Math.Round(GetTotalAffair(game,country)*GetAffairPower(game,country)/GetAffairDifficult(game,country)+10m/GetAreaNum(game,country),4);
		...
		internal static EArea? GetTargetArea(Game game) => game.PlayCountry?.MyApplyF(game.ArmyTargetMap.GetValueOrDefault);
		...
		internal static bool IsFocusDefense(Game game,ECountry? country) => country!=null&&game.ArmyTargetMap.TryGetValue(country.Value,out EArea? target)&&target==null;
	}
}
